...
ccnight 在任务 **开始 / 完成 / 失败 / 撞配额暂停（含恢复时间）** 四个时机发通知。
所有配置写在你自己的 `~/.config/ccnight/config.json`，与他人无关、不含任何共享凭证。
...
2. 写进 `~/.config/ccnight/config.json`：

```json
{ "webhook_url": "https://open.feishu.cn/open-apis/bot/v2/hook/xxxx" }
```

完事。ccnight 识别群机器人地址后自动使用飞书消息格式，无需其他配置。
...
4. 按界面引导定义参数。ccnight 发送的 JSON 长这样，可直接作为示例粘贴：

```json
{
  "source": "ccnight",
  "title": "ccnight: task done",
...
> 说明：此类 URL 走 ccnight 的 generic JSON 格式（即第 4 步那个结构），正好是流程参数需要的。
...
ccnight 每次通知会执行该命令，标题和内容通过环境变量传入：

- `$CCNIGHT_TITLE` — 如 `ccnight: task done`
- `$CCNIGHT_MESSAGE` — 如 `[3da094eb] refactor ...`

脚本里调什么、发给谁完全自定义；凭证由你的脚本自己管理，**不要写进 ccnight 的任何配置**。

验证命令（不消耗 Claude 配额）：

```bash
CCNIGHT_TITLE=test CCNIGHT_MESSAGE=hello /path/to/your-notify-script.sh
```
