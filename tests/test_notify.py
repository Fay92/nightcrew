...
from nightcrew import notify as notify_mod
...
def test_webhook_delivery(nightcrew_home, webhook_server, monkeypatch):
    url, received = webhook_server
    monkeypatch.setattr("sys.platform", "linux")  # skip the osascript branch
    cfg = Config(home=nightcrew_home, webhook_url=url)
    notify_mod.notify(cfg, "nightcrew: task done", "demo finished")
    assert len(received) == 1
    assert received[0]["source"] == "nightcrew"
    assert received[0]["title"] == "nightcrew: task done"
    assert received[0]["message"] == "demo finished"
    assert "timestamp" in received[0]


def test_webhook_failure_is_swallowed(nightcrew_home, monkeypatch, capsys):
    monkeypatch.setattr("sys.platform", "linux")
    cfg = Config(home=nightcrew_home, webhook_url="http://127.0.0.1:1/unreachable")
    notify_mod.notify(cfg, "t", "m")  # must not raise
    assert "webhook delivery failed" in capsys.readouterr().err


def test_macos_notification_invokes_osascript(nightcrew_home, monkeypatch):
    calls = []
    ...
    cfg = Config(home=nightcrew_home)
...
def test_non_macos_degrades_to_log_line(nightcrew_home, monkeypatch, capsys):
    monkeypatch.setattr("sys.platform", "linux")
    cfg = Config(home=nightcrew_home)
...
def test_webhook_payload_auto_detects_feishu():
    from nightcrew.notify import webhook_payload
...
def test_webhook_payload_generic_default_and_explicit_override():
    ...
    assert generic["source"] == "nightcrew" and generic["title"] == "t"
    forced = webhook_payload("https://example.com/hook", "feishu", "t", "m")
    assert forced["msg_type"] == "text"


def test_notify_command_receives_env(nightcrew_home, tmp_path):
    from nightcrew.config import Config
    from nightcrew.notify import notify

    out = tmp_path / "captured.txt"
    cfg = Config(
        home=nightcrew_home,
        notify_command=f'printf "%s|%s" "$NIGHTCREW_TITLE" "$NIGHTCREW_MESSAGE" > {out}',
    )
